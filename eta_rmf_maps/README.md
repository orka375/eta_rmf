...
These files include:
- A 2D map for the eta office world named *eta_office.png*
- An annotated file generated by the traffic editor named *eta_office.building.yaml*
...
```
colcon build --packages-select eta_rmf_maps
```
...
As stated before, the `eta_office.building.yaml` file was created using `traffic-editor`, which is part of the Open-RMF suite. The following steps outline the process followed to generate this file:

1.  **Obtain the Base Map**:
    *   A PNG image of the map is required. For this project, the PGM file from the `eta_gz` repository was converted to PNG format.
...
5.  **Configure Robot Spawn Points**:
    ...
    *   For each relevant vertex, set the robot's name (e.g., `eta1`) and type (e.g., `eta`).
